...
VSCode에서 로컬 Apache Tomcat 서버를 등록하고, 배포하고, 실행/디버그하고, JSP·Java 코드를 저장 즉시 반영해서 확인할 수 있게 해주는 확장 프로그램입니다.
...
- **한눈에 보이는 실행 상태**: 서버와 그 아래 배포된 각 앱 모두 `●`(실행 중/디버그 중) / `◐`(시작·중지 처리 중) / `○`(중지됨) 표시로 지금 실제로 서비스되고 있는지 바로 알 수 있습니다. 서버가 완전히 기동되면 알림으로도 안내됩니다.
...
- **배포**
  ...
  - **Deploy Exploded Folder...**: `WEB-INF` 를 포함한 빌드 출력 폴더를 그대로 가리키는 `context.xml`(`conf/Catalina/localhost/<app>.xml`, `docBase` 지정)을 생성합니다. 파일을 다시 빌드하면 war 재복사 없이 즉시 반영됩니다(`reloadable="true"`).
    ...
    - **Maven/Gradle 프로젝트 라이브 리로드**: `target/<artifactId>`(Maven) 또는 `build/exploded-<name>` 등(Gradle)처럼 빌드 산출물 폴더를 배포 대상으로 선택하면, 프로젝트 루트(`pom.xml`/`build.gradle`/`build.gradle.kts`/`settings.gradle(.kts)` 중 하나가 있는 가장 가까운 상위 폴더)를 기준으로 `src/main/webapp`(Maven·Gradle war 플러그인 공통 기본값)을 자동 감지하여 활성화할지 물어봅니다.
      - **파일 감시 후 즉시 복사**: 활성화하면 소스 폴더를 계속 감시하다가 JSP/HTML/CSS/JS 파일이 바뀌는 즉시 배포 폴더로 복사합니다. (초기 버전에서는 복사 대신 디렉터리 정션/심볼릭 링크로 소스를 직접 연결했었는데, Windows에서 `mvn clean` 실행 시 일부 도구가 정션을 통과해서 재귀 삭제하면서 **실제 소스 파일이 지워지는 심각한 문제**가 있어 복사 방식으로 되돌렸습니다. 안전을 위한 선택입니다.) `WEB-INF/classes`, `WEB-INF/lib` 는 항상 빌드 산출물 그대로 유지되고 건드리지 않습니다.
      ...
      - **수동 빌드**: 배포된 앱 우클릭 → **"Build Now (mvn/gradle)"** 로 언제든 원할 때 직접 빌드를 트리거할 수 있습니다("시작 전 빌드"와 같은 로직이라 Maven/Gradle 둘 다 자동 감지됩니다). 빌드 성공 후 `WEB-INF/classes` 로 동기화까지 한 번에 처리되고, 이 앱의 자동 컨텍스트 리로드가 꺼져 있으면(reloadable=false) 바로 이어서 "Reload Context Now" 를 실행할지 물어봅니다.
      - **시작 전 빌드 (기본 켜짐)**: 라이브 리로드가 켜진 Maven/Gradle 앱은, Tomcat 을 실제로 기동하기 **직전에 딱 한 번** 그 프로젝트의 컴파일 명령(`mvn compile -q` / `gradle classes -q`)을 실행하고 끝날 때까지 기다립니다 — 저장할 때마다가 아니라 시작 시점에만 실행되므로, 예전에 저장마다 빌드를 돌리면서 있었던 PATH·인코딩 문제가 훨씬 덜합니다. 이 서버의 **Set Java Home** 과 동일한 JDK 로 실행되고, 빌드가 실패해도 경고만 남기고 Tomcat 은 계속 시작됩니다. `tomcat.buildBeforeStart` 설정으로 끌 수 있고, `tomcat.mavenCommand`/`tomcat.gradleCommand` 로 실행 명령(래퍼 경로 등)을 바꿀 수 있습니다.
...
- **즉시 리로드 (Reload Context Now)**: 라이브 리로드가 켜진 앱은 자동 컨텍스트 리로드가 꺼져있으므로(위 참고), 필드·메서드·클래스 추가처럼 디버거 핫스왑이 처리 못 하는 구조적 변경을 반영하려면 이 명령이 필요합니다. 배포된 앱을 우클릭 → **"Reload Context Now (No Restart)"** 로 Tomcat **Manager API**를 통해 그 컨텍스트만 1~2초 내에 다시 로드합니다(전체 서버 재시작 없음). 라이브 리로드를 안 쓰는 일반 WAR 배포 등은 여전히 Tomcat의 `autoDeploy` 가 새/변경/삭제된 배포를 백그라운드에서 자동으로 처리합니다. 최초 사용 시 `conf/tomcat-users.xml`에 `manager-script` 권한을 가진 전용 계정을 자동 생성하며(비밀번호는 VSCode SecretStorage에만 저장, `settings.json`에는 절대 기록되지 않습니다), 이 계정이 활성화되려면 처음 한 번만 서버 재시작이 필요합니다(안내 메시지에서 바로 재시작할 수 있습니다). 이후로는 재시작 없이 계속 즉시 리로드를 사용할 수 있습니다. Tomcat 배포판에 `webapps/manager` 가 포함되어 있어야 합니다(표준 전체 배포판은 기본 포함). 인증에 실패(401)하면 서버 우클릭 → **"Reset Manager Credentials..."** 로 계정을 새로 만들고 재시작하면 됩니다 — "Reload Context Now" 실행 중 401이 뜨면 같은 동작을 바로 제안합니다.
- **핫스왑 실패 알림**: 디버그 모드에서 Java 저장 시 VSCode의 "Debugger for Java"가 자체적으로 핫스왑을 시도하는데, 그게 실패한 것으로 보이면(필드/메서드/클래스 추가 등 핫스왑이 원래 못 하는 변경일 때 흔함) 알림으로 안내하고 "Reload Context Now" 사용을 제안합니다. Java 디버거의 정확한 내부 이벤트 스펙까지는 확정할 수 없어 패턴 매칭 기반의 최선 노력(best-effort) 감지이며, 원본 이벤트는 서버 출력 채널에도 함께 기록됩니다.
- **Manager API 요청 타임아웃**: "Reload Context Now" 등은 응답을 기본 45초까지 기다립니다(빈이 많은 큰 애플리케이션은 컨텍스트 재로드 자체가 오래 걸릴 수 있어서입니다). `tomcat.managerRequestTimeoutSeconds` 설정으로 늘리거나 줄일 수 있습니다. 시간 초과가 나도 서버 쪽 리로드 자체는 계속 진행 중일 수 있으니, 콘솔 로그로 실제 완료 여부를 확인해보세요.
...
- **Java 버전(JAVA_HOME) 설정**: 서버 우클릭 → **Set Java Home...** 으로 이 Tomcat 인스턴스가 사용할 JDK 폴더를 지정할 수 있습니다(설정하지 않으면 시스템 기본 JAVA_HOME 사용). 여러 서버마다 다른 JDK를 지정해 서버별로 Java 버전을 분리할 수 있습니다.
...
- **(0.9.0) Windows에서 "'chcp' 은(는) 내부 또는 외부 명령이 아닙니다" 로 빌드가 항상 실패하던 문제 — 수정됨**: 콘솔 인코딩을 맞추려고 `chcp 65001>nul && <실제 빌드 명령>` 식으로 명령을 연결했었는데, `&&` 는 앞 명령이 성공해야만 뒤를 실행하는 연산자라 `chcp` 가 어떤 이유로든 실패하면(예: PATH에서 System32 가 빠진 환경) `mvn`/`gradle` 자체는 PATH에 정상적으로 있어도 전혀 실행되지 않았습니다. 이제 실패해도 항상 다음 명령이 실행되는 `&` 로 바꿨고, Windows에서 System32 경로가 PATH에서 빠져 있으면 방어적으로 보충해서 추가합니다.
- **"path" 속성 무시 경고**: Tomcat 7 이후로 `conf/Catalina/<host>/*.xml` 컨텍스트 파일은 **파일명**으로 경로가 결정되고, 파일 안의 `path` 속성은 무시됩니다(Tomcat 9+ 에서는 배포마다 경고 로그를 남깁니다). 0.7.0 이전 버전에서 생성된 `context.xml` 에는 이 속성이 남아있을 수 있습니다 — 해당 앱을 우클릭 → **Enable Live Source Reload (Maven/Gradle)...** 를 다시 실행하면(라이브 리로드가 이미 켜져 있어도 그대로 다시 실행) `path` 속성 없이 재생성되어 경고가 사라집니다.
- **(0.7.x 이전) `mvn clean` 실행 시 `src/main/webapp` 파일이 삭제되던 문제 — 수정됨**: 한때 라이브 소스 리로드가 디렉터리 정션/심볼릭 링크로 소스 폴더를 배포 폴더에 직접 연결했는데, Windows에서 일부 재귀 삭제 로직(Maven `clean` 플러그인 포함)이 정션을 링크로 인식하지 못하고 통과해서 재귀 삭제하는 바람에 **실제 소스 파일이 지워지는 문제**가 있었습니다. 지금은 링크 대신 파일을 복사하는 방식으로 되돌려서 이 문제가 근본적으로 발생할 수 없습니다. 예전 버전에서 이미 이 문제를 겪으셨다면 `git status`로 `src/main/webapp` 변경 이력을 확인해 복구해주세요.
- **Java 저장 후 "Applying code changes"가 오래 걸림**: 이건 이 확장의 기능이 아니라 **VSCode의 "Debugger for Java" 확장이 하는 핫스왑(코드 교체) 작업**입니다. 대규모 프로젝트에서 증분 컴파일·클래스 재정의 검증에 시간이 걸리거나, Windows 백신 프로그램이 파일 쓰기마다 검사하면서 느려지는 경우가 흔합니다(프로젝트/JDK 폴더를 백신 예외 목록에 추가하면 크게 개선되는 경우가 많습니다). 이 확장이 관여하는 지점이 아니라 직접 손볼 수 있는 부분은 없습니다.
